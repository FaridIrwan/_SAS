...
Partial Class RoomType
    ...
    Dim CFlag As String
    Dim DFlag As String
    Dim ListObjects As List(Of RoomTypeEn)
...
#Region "Methods"
    ...
    Private Sub LoadUserRights()
        ...
        Catch ex As Exception
            LogError.Log("RoomType", "LoadUserRights", ex.Message)
        End Try
        'Rights for Add
    ...
    ''' <summary>
    ''' Method to Save and Update RoomType
    ''' </summary>
    ''' <remarks></remarks>
    Private Sub OnSave()
        Dim bsobj As New RoomTypeBAL
        Dim eobj As New RoomTypeEn
        Dim RecAff As Integer

        eobj.SART_Code = Trim(txtRoomTypeCode.Text)
        eobj.SART_Description = Trim(txtRoomTypeDesc.Text)
        If ddlblock.SelectedValue = "-1" Then
            lblMsg.Text = "Select kolej"
            Exit Sub
        Else
        ...
        If Session("PageMode") = "Add" Then
            ...
            Catch ex As Exception
                ...
                LogError.Log("RoomType", "OnSave", ex.Message)
            End Try
    ...
    Private Sub OnDelete()
        ...
        If txtRoomTypeCode.Text <> "" Then
            ...
            If lblCount.Text > 0 Then
                ...
                Dim RecAff As Integer
                lblMsg.Visible = True
                eobj.SART_Code = txtRoomTypeCode.Text
                Try
                ...
                Catch ex As Exception
                    lblMsg.Text = ex.Message.ToString()
                    LogError.Log("RoomType", "OnDelete", ex.Message)
                End Try
                txtRoomTypeCode.Text = ""
                txtRoomTypeDesc.Text = ""
                ddlblock.SelectedValue = "-1"
    ...
    End Sub
    ''' <summary>
    ''' Method to Get List of RoomType and Load
    ''' </summary>
    ''' <remarks></remarks>
    Public Sub LoadListObjects()
        Dim eob As New RoomTypeEn
        Dim bobj As New RoomTypeBAL

        If txtRoomTypeCode.Text = "" Then txtRoomTypeCode.Text = "*"
        eob.SART_Code = Trim(txtRoomTypeCode.Text)
        eob.SART_Description = Trim(txtRoomTypeDesc.Text)
        If ddlblock.SelectedValue = "-1" Then
            eob.SABK_Code = "*"
        Else
            eob.SABK_Code = ddlblock.SelectedValue
        End If
        If ddlKolej.SelectedValue = "-1" Then
            eob.SAKO_Code = "*"
        Else
            eob.SAKO_Code = ddlKolej.SelectedValue
        End If
        Try
            ListObjects = bobj.GetRoomTypeListall(eob)
        Catch ex As Exception
            LogError.Log("RoomType", "LoadListObjects", ex.Message)
        End Try
        Session("ListObj") = ListObjects
    ...
    Private Sub FillData(ByVal RecNo As Integer)
        ...
        Else
            'PnlView.Visible = False
            'PnlAdd.Visible = True
            Dim obj As RoomTypeEn
            ListObjects = Session("ListObj")
            obj = ListObjects(RecNo)

            txtRoomTypeCode.Text = obj.SART_Code
            txtRoomTypeDesc.Text = obj.SART_Description
    ...
    Private Sub Menuname(ByVal MenuId As Integer)
        ...
        Catch ex As Exception
            LogError.Log("RoomType", "Menuname", ex.Message)
        End Try
