...
Imports System.Data
Partial Class semesterStup
    Inherits System.Web.UI.Page
    Dim CFlag As String
    Dim DFlag As String
    Dim ListObjects As List(Of SemesterSetupEn)
...
#Region "Methods"
    ...
    Private Sub LoadUserRights()
        ...
        Catch ex As Exception
            LogError.Log("SemesterSetUp", "LoadUserRights", ex.Message)
        End Try
        'Rights for Add
    ...
    End Sub
    ''' <summary>
    ''' Method to Save and Update SemesterSetUps 
    ''' </summary>
    ''' <remarks></remarks>
    Private Sub OnSave()
        Dim bsobj As New SemesterSetupBAL
        Dim eobj As New SemesterSetupEn
        Dim RecAff As Integer
        eobj.SemisterSetupCode = Trim(txtSemesterCode.Text)
        eobj.Semester = Trim(txtSem.Text)

        eobj.SemisterCode2 = Trim(txtSemester.Text)
        ...
        If ddlCurSem.SelectedValue = 0 Then
            eobj.CurrSem = False
        Else
            eobj.CurrSem = True
        End If
        ...
        If Session("PageMode") = "Add" Then
            ...
            Catch ex As Exception
                ...
                LogError.Log("SemesterSetUp", "OnSave", ex.Message)
            End Try
    ...
    End Sub
    ''' <summary>
    ''' Method to Delete the SemesterSetUps
    ''' </summary>
    ''' <remarks></remarks> 
    Private Sub OnDelete()
        If txtSemesterCode.Text.Length <> 0 Then
            If lblCount.Text = "" Then lblCount.Text = 0
            If lblCount.Text > 0 Then
                Dim bsobj As New SemesterSetupBAL
                Dim eobj As New SemesterSetupEn
                Dim RecAff As Integer
                eobj.SemisterSetupCode = txtSemesterCode.Text
                ...
                Catch ex As Exception
                    lblMsg.Text = ex.Message.ToString()
                    lblMsg.Visible = True
                    LogError.Log("SemesterSetUp", "OnDelete", ex.Message)
                End Try
                txtSemesterCode.Text = ""
                txtSemester.Text = ""
                ddlStatus.SelectedValue = "1"
    ...
    End Sub
    ''' <summary>
    ''' Method to get the List Of SemesterSetUp
    ''' </summary>
    ''' <remarks></remarks>
    Public Sub LoadListObjects()
        Dim ds As New DataSet
        Dim bobj As New SemesterSetupBAL
        ...
        eobj.CurrSem = ddlCurSem.SelectedValue

        Try
            ListObjects = bobj.GetSemesterSetupList(eobj)
        Catch ex As Exception
            LogError.Log("SemesterSetUp", "LoadListObjects", ex.Message)
        End Try
        Session("ListObj") = ListObjects
    ...
    Private Sub FillData(ByVal RecNo As Integer)
        ...
        Else
            ...
            Else
                PnlView.Visible = False
                PnlAdd.Visible = True
                Dim obj As SemesterSetupEn
                ListObjects = Session("ListObj")
                obj = ListObjects(RecNo)

                txtSemesterCode.Text = obj.SemisterSetupCode
                txtSemester.Text = obj.SemisterCode2
                txtSem.Text = obj.Semester
                If obj.Status = True Then
                    ddlStatus.SelectedValue = 1
                Else
                    ddlStatus.SelectedValue = 0
                End If
                If obj.CurrSem = True Then
                    ddlCurSem.SelectedValue = 1
                Else
                    ddlCurSem.SelectedValue = 0
                End If
            End If
        End If
    End Sub
    ...
    Private Sub Menuname(ByVal MenuId As Integer)
        ...
        Catch ex As Exception
            LogError.Log("SemesterSetUp", "Menuname", ex.Message)
        End Try
