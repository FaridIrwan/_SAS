...
#Region "Methods"
    ...
    Private Sub LoadInvoiceGrid()
        ...
        Else
            ...
            For Each dgItem1 In dgInvoices.Items
                ...
                If cat = "Invoice" Then
                    link.NavigateUrl = "../BatchInvoice.aspx?Menuid=14&Formid=Inv&IsStudentLedger=1&BatchCode=" + str + ""
                    link.Target = "MyPopup"
                ...
                ElseIf cat = "Refund" Then
                    link.NavigateUrl = "../Payments.aspx?Menuid=19&BatchCode=" + str + ";St"
                    link.Target = "MyPopup"
                ElseIf cat = "SPA" Then
                    link.NavigateUrl = "../SponsorAllocation.aspx?Menuid=25&BatchCode=" + str + ""
                    link.Target = "MyPopup"
                ElseIf cat = "AFC" Then
                    link.NavigateUrl = "../FeePosting.aspx?Menuid=73&BatchCode=" + str + ""
                    link.Target = "MyPopup"
                ElseIf cat = "Credit Note" Then
                    link.NavigateUrl = "../BatchInvoice.aspx?Menuid=16&Formid=CN&IsStudentLedger=1&BatchCode=" + str + ""
                    link.Target = "MyPopup"
                ElseIf cat = "Debit Note" Then
                    link.NavigateUrl = "../BatchInvoice.aspx?Menuid=15&Formid=DN&IsStudentLedger=1&BatchCode=" + str + ""
                    link.Target = "MyPopup"
                ElseIf cat = "STA" Then
                    link.NavigateUrl = "../Payments.aspx?Menuid=19&&BatchCode=" + str + ";A"
                    link.Target = "MyPopup"
    ...
    End Sub
    ''' <summary>
    ''' Method to Set Grid to Ledger Format
    ''' </summary>
    ''' <remarks></remarks>
    ...
    '    If rdbStudentLeddger.Checked = False Then

    '        Dim i As Double = 0
    '        i = txtDebitAmount.Text
    '        txtDebitAmount.Text = txtCreditAmount.Text
    '        txtCreditAmount.Text = i
    ...
    Private Sub ledgerformat()
        ...
        If rdbStudentLeddger.Checked = False Then
            ...
            'Add decimal for Total Credit Amount textbox - START
            txtCreditAmount.Text = String.Format("{0:F}", i)
            'Add decimal for Total Credit Amount textbox - END
        ...
        If rdbStudentLeddger.Checked = True Then
            Dim rb As String = "stud_ledger"

            Try
                _AccountsDAL.AutoUpdateStudOutstanding(argStudent, CDbl(txtOutAmt.Text), rb)
        ...
        Else
            Dim rb As String = "loan_ledger"

            Try
                _AccountsDAL.AutoUpdateStudOutstanding(argStudent, CDbl(txtOutAmt.Text), rb)

            Catch ex As Exception
                LogError.Log("LoanLedger", "AutoUpdate Function", ex.Message)
            End Try
    ...
    Private Sub Loanledgerformat()
        ...
        For Each dgItem1 In dgInvoices.Items
            If dgItem1.Cells(6).Text = "Credit" Then
                ...
                dgItem1.Cells(7).Text = String.Format("{0:F}", amount) & "-"
                cr = cr + amount
                txtDebitAmount.Text = -(String.Format("{0:F}", cr))
            Else
                TotalAmount = TotalAmount + CDbl(dgItem1.Cells(7).Text)
                dgItem1.Cells(8).Text = String.Format("{0:F}", TotalAmount)
                amount = dgItem1.Cells(7).Text
                dgItem1.Cells(7).Text = String.Format("{0:F}", amount) & "+"
                dr = dr + amount
                txtCreditAmount.Text = String.Format("{0:F}", dr)
            End If
            If dgItem1.Cells(0).Text = "01/01/0001" Then
                dgItem1.Cells(0).Text = "--"
            End If
